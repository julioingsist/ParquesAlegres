		...
		switch ($value['type']) {
			...
			case "top-socials":
				...
				foreach($value['options'] as $key) {
					$header .= '<li class="social"><a href="' .$key[3]. '"><img class="normal" src="' .MP_ROOT.'/massive-panel/images/icons/social/' . $key[0] . '"/><img class="hover" src="' .MP_ROOT.'/massive-panel/images/icons/social/' . $key[1] . '"/></a><span class="mp-tooltip"><span class="arrow"></span>' . $key[2] . '</span></li>';
				}
...
function get_books_table() {
	...
	for($i = 0; $i < $books_count; $i++) {
		...
		if(isset($settings['books'][$i]['pages'][0]) && $settings['books'][$i]['pages'][0]['rfbwp_fb_page_bg_image'] != '')
			...
			// $output .= '<img class="img-border" src="'.MP_ROOT.'/massive-panel/timthumb.php?src='.$settings['books'][$i]['pages'][0][$shortname.'_fb_page_bg_image'].'&w=0&h=137&zc=0" alt=""/>';
		else

...
/*if ( !empty($_GET['ajax']) && $_GET['ajax'] == true ) {
      ...
      <h3><?php the_title();?></h3>
      <p>Giro: <?php echo get_post_meta($post->ID, "_parque_ubic", true); ?></p>
      <p>Descripción: <?php echo get_post_meta($post->ID, "_parque_col", true); ?></p>
      <p>Correo: <?php echo get_post_meta($post->ID, "_parque_sup", true); ?></p>
      <p>Teléfono: <?php echo get_post_meta($post->ID, "_parque_sec", true); ?></p>
      <p>Contacto: <?php echo get_post_meta($post->ID, "_parque_tipo", true); ?></p>
      <p>Dirección: <?php echo get_post_meta($post->ID, "_parque_cont", true); ?></p>
      <p>Dirección: <?php echo get_post_meta($post->ID, "_parque_ciudad", true); ?></p>
      <p>Dirección: <?php echo get_post_meta($post->ID, "_parque_estado", true); ?></p>
      <p>Dirección: <?php echo get_post_meta($post->ID, "_parque_cve_exp", true); ?></p>
      <p>Dirección: <?php echo get_post_meta($post->ID, "_parque_plano", true); ?></p>
      <p>Dirección: <?php echo get_post_meta($post->ID, "_parque_gallery", true); ?></p>
      <a href="<?php echo get_permalink(); ?>" class="link link--natural">ver más</a>
...
<?php
$parque = get_the_ID();
$b = html_entity_decode(get_post_meta($post->ID, "_parque_maps", true));
        echo'<table><tr><th>Nombre de parque:</th><th align="center">'.$post->post_title.'</th></tr>
          <tr><th>Ubicaci&oacute;n</th><td>'.get_post_meta($post->ID, "_parque_ubic", true).'</td></tr>
          <tr><th>Colonia</th><td>'.get_post_meta($post->ID, "_parque_col", true).'</td></tr>
          <tr><th>Superficie</th><td>'.get_post_meta($post->ID, "_parque_sup", true).' m&#178;</td></tr>
          <tr><td style="padding:0;" align="center" colspan=2 >'.$b.'</td></tr>
          <tr><th>Sector</th><td>'.get_post_meta($post->ID, "_parque_sec", true).'</td></tr>
          <tr><th>Tipo</th><td>'.get_post_meta($post->ID, "_parque_tipo", true).'</td></tr>
          <tr><th>Contacto de comit&eacute;</th><td>'.get_post_meta($post->ID, "_parque_cont", true).'</td></tr>
          <tr><th>Ciudad</th><td>'.get_post_meta($post->ID, "_parque_ciudad", true).'</td></tr>
          <tr><th>Estado</th><td>'.get_post_meta($post->ID, "_parque_estado", true).'</td></tr>
	  <tr><th>Experiencias Exitosas</th><td>'.get_post_meta($post->ID, "_parque_expexitosa", true).'</td></tr>
          </table>';
	  //-----------------------Comienza Galerias
	...
	//     -----------------------------------------------------
	 ...
	 echo'<tr><td>CUENTA CON VISI&Oacute;N DEL ESPACIO</td>';
      ...
      echo' <td>';
            ...
            //---------------------------------------
            if($row3['vespacio']){
               echo' <div class="create-tooltip" title="'.$row3['vespacio'].'" ><font '.$color.'>'.$row2['vespacio'].'</font></div>';
            }else{
               echo$row2['vespacio'];
            }
            echo'</td>';

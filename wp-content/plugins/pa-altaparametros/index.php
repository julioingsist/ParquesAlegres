 ...
 */
/*
Plugin Name: PA Formulario para agregar parametros a un parque
...
function my_plugin_menu4() {
        add_menu_page( 'Alta Parámetros', 'Alta Parámetros', 'edit_posts', 'Alta Parámetros', 'altaparametros_tres' );
}
...
$sql="select * from wp_comites_parques where cve=$visita";
	$res=mysql_query($sql);
	$sqli="SELECT id as cve, post_title as nom FROM  `wp_posts` WHERE id=$parque and post_type='parque' AND post_status='publish' order by post_title desc";
...
echo '<tr><th>Fecha de la visita</th><td><input type="text" id="datepicker" value="'.$row['fecha_visita'].'" name="fecha_visita"/></td></tr>';
echo'<tr>';
...
echo'<td>';
if($row['disenio']==40){
echo'<input type="checkbox" value="40" name="disenio" checked>Dise&ntilde;o ';
}else{
echo'<input type="checkbox" value="40" name="disenio">Dise&ntilde;o ';
}
if($row['ejecutivo']==40){
echo'<input type="checkbox" value="40" name="ejecutivo" checked>Ejecutivo ';
}else{
echo'<input type="checkbox" value="40" name="ejecutivo">Ejecutivo ';
}
if($row['vespacio']==40){
echo'<input type="checkbox" value="40" name="vespacio" checked>Visi&oacute;n del espacio';
}else{
echo'<input type="checkbox" value="40" name="vespacio">Visi&oacute;n del espacio';
}
echo'</td>';
echo'</tr>';
echo'<tr>';
...
{
add_submenu_page( 'edit.php?post_type=parque', __( "Alta parámetros", "altaparametros" ), __( "Alta parámetros", "altaparametros" ), 'edit_posts', 'altaparametros', 'altaparametros_tres' );
}

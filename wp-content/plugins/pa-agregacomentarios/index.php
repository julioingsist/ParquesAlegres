 ...
 */
/*
Plugin Name: PA formulario comentarios de parametros
...
function my_plugin_menu5() {
        add_menu_page( 'Comentarios Parámetros', 'Comentarios Parámetros', 'edit_posts', 'Comentarios Parámetros', 'agregarcomentarios_tres' );
}
...
$res=mysql_db_query("parquesa_ParquesAlegresWP",$sql);
$sql2="SELECT * FROM  `wp_comites_parques` WHERE cve_parque=$parque and cve=$visita";
$res2=mysql_db_query("parquesa_ParquesAlegresWP",$sql2);
$row2=@mysql_fetch_array($res2);
$row=@mysql_fetch_array($res);
$sql3="SELECT * FROM  `wp_visitascom_parques` WHERE cve_parque=$parque and cve_visita=$visita";
$res3=mysql_db_query("parquesa_ParquesAlegresWP",$sql3);
$row3=@mysql_fetch_array($res3);
$sql4="SELECT *  FROM wp_comites_parques WHERE cve_parque =$parque AND cve <$visita GROUP BY cve DESC LIMIT 1";
$res4=mysql_db_query("parquesa_ParquesAlegresWP",$sql4);
$row4=@mysql_fetch_array($res4);
//echo$sql2;
//$cve_visita=$row2['cve'];

if ($parque){ $f="Parque: <b>$parque</b><br>";echo$row['post_title']; }else{ header('Location: http://parquesalegres.org/wp-admin/edit.php?post_type=parque');
$f='<b>Debe seleccionar la palabra "comentarios" del listado de visitas del parque dando <a href="http://parquesalegres.org/wp-admin/admin.php?page=Par%C3%A1metros%20Parque">click aqui</a></b><br>';}
echo $f;
echo'</center>';
$row=@mysql_fetch_array($res);
...
echo'</th>';
echo'<th colspan="1">visita anterior';
echo'</th>';
...
echo'</td>';
echo'<td align="center">'.$row4['opera'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['formaliza'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['organiza'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['reunion'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['proyecto'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['disenio'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['ejecutivo'].'</td>';
...
echo'</tr>';
echo'</tr>';
echo'<td>Cuenta con Visi&oacute;n del espacio:';
echo'</td>';
echo'<td align="center">'.$row4['vespacio'].'</td>';
echo'<td align="center">'.$row2['vespacio'].'</td>';
echo'<td><textarea style="width:300px;height:100px;" name="vespacio">'.$row3['vespacio'].'</textarea>';
echo'</td>';
echo'</tr>';
echo'<tr>';
echo'<td>Esta en buen estado lo que existe:';
echo'</td>';
echo'<td align="center">'.$row4['estado'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['instalaciones'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['ingresop'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['ingresadop'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['mancomunado'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['eventosr'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['eventos'].'</td>';
...
echo'</td>';
if($row4['averdes']==17){
echo'<td align="center">'.$row4['averdes'].'</td>';
	     }else{
		  if($row4['averdes']<50){
			if($row4['averdes']==0){
			$averdes=0;
		  echo'<td align="center">'.$averdes.'</td>';			
			}else{
		  $averdes=34;
		  echo'<td align="center">'.$averdes.'</td>';
			}
...
echo'</td>';
echo'<td align="center">'.$row4['estaver'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['gente'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['respint'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['orden'].'</td>';
...
echo'</td>';
echo'<td align="center">'.$row4['limpieza'].'</td>';
...
echo'</td>';
echo'</tr>';
$tipos_visita = array("---Sin Tipo---","Visita de reforzamiento","Visita de seguimiento","Visita de evento","Visita de prospecto");
echo '<tr><th>Tipo de visita</th><td>'.$tipos_visita[$row4["tipo_visita"]].'</td><td><select name="tipo_visita">';
//echo'<option value="selecciona">---Selecciona Tipo---</option>';
for($i=0; $i<=4; $i++){
echo'<option value="'.$i.'"';
if($row3["tipo_visita"]==$i){
echo'selected';
}
...
echo'</td>';
echo'<td align="center" colspan="2">'.$row2['genvisita'].'</td>';
...
//		echo'<A href="http://parquesalegres.org/parques/evaluacion/">Ir al listado de parques</a>';
       ...
       $sSQL="INSERT INTO `wp_visitascom_parques`(`cve_parque`,`cve_visita`, `opera`, `formaliza`, `organiza`, `reunion`, `proyecto`,`disenio`,`ejecutivo`,`vespacio`, `instalaciones`, `estado`, `ingresop`, `ingresadop`, `mancomunado`, `eventos`, `eventosr`, `averdes`, `estaver`,  `gente`,  `limpieza`, `orden`, `respint`, `genvisita`, `tipo_visita`) VALUES ('$parque','$_POST[visita]','$_POST[opera]','$_POST[formaliza]','$_POST[organiza]','$_POST[reunion]','$_POST[proyecto]','$_POST[disenio]','$_POST[ejecutivo]','$_POST[vespacio]','$_POST[instalaciones]','$_POST[estado]','$_POST[ingresop]','$_POST[ingresadop]','$_POST[mancomunado]','$_POST[eventos]','$_POST[eventosr]','$_POST[averdes]','$_POST[estaver]','$_POST[gente]','$_POST[limpieza]','$_POST[orden]','$_POST[respint]','$_POST[genvisita]','$_POST[tipo_visita]')";
		mysql_db_query("parquesa_ParquesAlegresWP",$sSQL);
		//echo$sSQL;
		//afluencia contra lo posible `diario`,'$_POST[diario]'
		echo'<p align="center">';
...
{
add_submenu_page( 'edit.php?post_type=parque', __( "Agrega comentarios", "agregacomentarios" ), __( "Agrega comentarios", "agregacomentarios" ), 'edit_posts', 'agregacomentarios', 'agregarcomentarios_tres' );
}

...
if(mysql_num_rows($res)>0){
    ...
    while($row=mysql_fetch_array($res)){
        if ( get_post_meta( $row['ID'], '_parque_gallery', true )){
            echo '<tr><td>'.$row['ID'].'</td><td><a href="'.$row['guid'].'" target="_blank">'.$row['post_title'].'</a></td><td><table><tr>';
    ...
    echo '<tr><th colspan="2">Total parques: '.$i.'</th><th>Total imagenes: '.$img.'</th></tr></table>';
}
echo '</body></html>';
?>